        ...
        if (bits_ >= 160) {
            ...
            */
            ...
            if (data_[0] != 0x11 || data_[1] != (SENSOR_ID & 0xFF) || data_[3] != 0x07 || data_[4] != 0x0E || rcv_sensor_id != SENSOR_ID) {
                ...
                m += sprintf(m,"{time:%ld,seqID:%d,power:%f,pulse:%d,batt:%d,FreqErr:%.2f",mytime,seq, watt, pulse, battery, freq);
...
int run_calibration(FILE *f){
    ...
    fprintf(stdout, "SPARSNAS_FREQ_MIN=%f SPARSNAS_FREQ_MAX=%f\n", best_f1, best_f1 + 40000.0f);
    return 0;
}
